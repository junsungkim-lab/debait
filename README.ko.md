...
> **여러 AI가 토론하고, 가장 좋은 답 하나를 냅니다.**

AI 하나에게 묻는 대신, Debait은 구조화된 토론을 실행합니다. **Solver**가 제안하고, **Critic**이 반박하고, **Checker**가 검증하고, **Synth**가 최종 정제된 답을 만듭니다. 로그인 없이 바로 브라우저에서 사용 가능합니다.
...
> 대화 기록과 API 키는 `app.db` 파일에 로컬 저장되어 재시작해도 유지됩니다.

---

## 🎯 왜 Debait인가?

| | ChatGPT 단독 | AutoGen / CrewAI | **Debait** |
|--|--|--|--|
| 설치 | 즉시 | 복잡한 설정 | **명령어 3줄** |
| 토론 역할 | ❌ | 커스텀 에이전트 | **내장 (Solver/Critic/Checker/Synth)** |
| 웹 UI | ❌ | ❌ | **✅ 기본 포함** |
| Telegram | ❌ | ❌ | **✅ 기본 포함** |
...
키는 **Fernet(AES-128)**으로 암호화 저장 — 평문 저장 없음

### 3. 역할별 모델 조합 (선택)

```
Solver  → groq:llama-3.3-70b-versatile   # 빠름
...
*Debait이 도움이 됐다면 ⭐ 스타를 눌러주세요. 더 많은 사람들이 찾을 수 있게 됩니다.*
